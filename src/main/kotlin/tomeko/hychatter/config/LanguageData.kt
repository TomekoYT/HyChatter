...
object LanguageData : DataProvider {
    ...
    @Volatile private var fetchedComponents: Map<String, IChatComponent> = emptyMap()
    ...
    fun load(showNotification: Boolean) {
        try {
            ...
            val components = mutableMapOf<String, IChatComponent>()
            response.asJsonObject?.getAsJsonObject("components")?.entrySet()?.forEach { entry ->
                runCatching { IChatComponent.Serializer.jsonToComponent(entry.value.toString()) }
    ...
    private fun component(default: IChatComponent) = ReadOnlyProperty<Any?, IChatComponent> { _, property ->
